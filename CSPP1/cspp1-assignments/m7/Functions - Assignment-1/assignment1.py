...
def paydebt_offinyr(balance, annual_interest_rate, monthly_payment_rate):
    """ parameter on which the required calculations done"""
    previous_balance = balance
    for _ in range(12):
        mntly_irate = (annual_interest_rate) / 12.0
        mini_monthpay = (monthly_payment_rate) * (previous_balance)
        mntly_unpbal = (previous_balance) - (mini_monthpay)
...
def main():
    ...
    print("Remaining balance:", paydebt_offinyr(data[0], data[1], data[2]))
